...
namespace HMS
{
    public partial class doctor_list : Form
    {
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            {
                using (OleDbCommand deleteCommand = new OleDbCommand("DELETE FROM doctor WHERE [ID] = ?", connection)) //حذف کردن
                {
                    connection.Open();
        ...
        private void load_doctor_grid_view()
        {
            ...
            using (OleDbConnection connection = new OleDbConnection(string.Format("Provider=Microsoft.Jet.OLEDB.4.0;Data Source={0}", mdfFile)))
            {
                using (OleDbCommand selectCommand = new OleDbCommand("SELECT * FROM doctor", connection))
                {
                    ...
                    dataGridView1.Columns[6].HeaderCell.Value = "تخصص";
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            using (OleDbConnection connection = new OleDbConnection(string.Format("Provider=Microsoft.Jet.OLEDB.4.0;Data Source={0}", mdfFile)))
            {
                ...
                using (OleDbCommand selectCommand = new OleDbCommand("SELECT * FROM doctor where [nationalcode]='" + nationalcode + "' and [id] not in (" + id + ")", connection))
                {
                    connection.Open();
                    ...
                    if (!reader.HasRows)
                    {
                        using (OleDbCommand updateCommand = new OleDbCommand("UPDATE doctor SET [username] = ?, [userfamily] = ? ,[fathername] = ?, [nationalcode] = ?,[userage] = ?,[title]=? WHERE [ID] = ?", connection)) //ویرایش
                        {
                            string name = "";
                            string family = "";
                            string father = "";
                            ...
                            try
                            {
                                ...
                                title = dataGridView1.CurrentRow.Cells[6].Value.ToString();
                            }
                            catch
                            {
                    ...
                    //قطع کردن ارتباط 
                    connection.Close();
                    //بالا اوردن گرید ویو مربوط به دکتر 
                    load_doctor_grid_view();
                    //رفرش کردن اطلاعات مربوط به دیتا در گرید ویو به علت وجود دستور اپدیت
                    dataGridView1.Refresh();
                }
            }
        }
